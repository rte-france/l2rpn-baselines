...
def to_pad_vect(inputv, pad_w, pad_v = 0.0):
    v = np.asarray(inputv)
...
def convert_obs_pad(obs):
    ...
    # Get time data
    time_li = [obs.month, obs.day, obs.day_of_week,
               obs.hour_of_day, obs.minute_of_hour]
    time_v = to_pad_vect(time_li, pad_w)
    time_line_cd = to_pad_vect(obs.time_before_cooldown_line, pad_w, pad_v=-1.0)
    time_line_reco = to_pad_vect(obs.time_before_line_reconnectable, pad_w, pad_v=-1.0)
    time_line_nm = to_pad_vect(obs.time_next_maintenance, pad_w)
    time_sub_cd = to_pad_vect(obs.time_before_cooldown_sub, pad_w, pad_v=-1.0)
    
    # Get generators info
    g_p = to_pad_vect(obs.prod_p, pad_w)
    g_q = to_pad_vect(obs.prod_q, pad_w)
    g_v = to_pad_vect(obs.prod_v, pad_w)
    g_tr = to_pad_vect(obs.target_dispatch, pad_w)
    g_ar = to_pad_vect(obs.actual_dispatch, pad_w)
    g_buses = np.zeros(obs.n_gen)
    for gen_id in range(obs.n_gen):
        g_buses[gen_id] = topo[g_pos[gen_id]]
        if g_buses[gen_id] <= 0.0:
            g_buses[gen_id] = 0.0
    g_bus = to_pad_vect(g_buses, pad_w, pad_v=-1.0)

    # Get loads info
    l_p = to_pad_vect(obs.load_p, pad_w)
    l_q = to_pad_vect(obs.load_q, pad_w)
    l_v = to_pad_vect(obs.load_v, pad_w)
    l_buses = np.zeros(obs.n_load)
    for load_id in range(obs.n_load):
        l_buses[load_id] = topo[l_pos[load_id]]
        if l_buses[load_id] <= 0.0:
            l_buses[load_id] = 0.0
    l_bus = to_pad_vect(l_buses, pad_w, pad_v=-1.0)

    # Get lines origin info
    or_p = to_pad_vect(obs.p_or, pad_w)
    or_q = to_pad_vect(obs.q_or, pad_w)
    or_v = to_pad_vect(obs.v_or, pad_w)
    ...
    or_bus = to_pad_vect(or_buses, pad_w, pad_v=-1.0)
    or_rho = to_pad_vect(obs.rho, pad_w, pad_v=-1.0)
    
    # Get extremities origin info
    ex_p = to_pad_vect(obs.p_ex, pad_w)
    ex_q = to_pad_vect(obs.q_ex, pad_w)
    ex_v = to_pad_vect(obs.v_ex, pad_w)
    ex_buses = np.zeros(obs.n_line)
    for line_id in range(obs.n_line):
        ex_buses[line_id] = topo[lex_pos[line_id]]
        if ex_buses[line_id] <= 0.0:
            ex_buses[line_id] = 0.0
    ex_bus = to_pad_vect(ex_buses, pad_w, pad_v=-1.0)
    ex_rho = to_pad_vect(obs.rho, pad_w, pad_v=-1.0)

    res = np.stack([
        # [0;4] Time
        time_v, time_line_cd, time_sub_cd, time_line_reco, time_line_nm,
        # [5;10] Gens
